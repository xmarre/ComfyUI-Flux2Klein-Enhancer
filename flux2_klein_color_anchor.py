...
WHY THIS EXISTS:
  The reference latent gives the model accurate color at step 0 (high sigma).
  As sampling progresses and sigma drops, the model's learned prior gradually
  dominates and bleeds its own color statistics over the reference — the node
  wins early, the model wins late.

HOW IT WORKS:
  Hooks into the sampler's post-CFG callback. After every denoising step,
  it measures the drift between the model's predicted x0 channel means and
  the reference latent's channel means, then nudges the prediction back.

  ONLY the per-channel spatial mean (DC offset = color) is corrected.
  The spatial deviation from that mean (structure, edges, texture) is
  completely untouched — so this cannot smear or destroy image detail.

  Correction = (ref_mean - predicted_x0_mean) * effective_strength

  effective_strength is driven by the HIGHER of two progress signals:
    1. sigma-based:  (sigma_max - current_sigma) / sigma_max
    2. step-based:   1 - 0.5^step_number  (0.5, 0.75, 0.875, 0.9375, …)

  Taking the max means the ramp grows reliably even in 4-step schedules where
  the sigma range is narrow (e.g. 0.97→0.77) and sigma-only progress would
  stay below 0.25 for the entire run.

  BUG THAT WAS FIXED (ramp_curve tooltip was backwards):
    progress^(1/curve) with curve>1 gives a sub-linear (FAST-START) curve.
    progress^(1/curve) with curve<1 gives a super-linear (SLOW-START) curve.
    The old tooltip had these descriptions swapped. The math and behaviour
    are unchanged; only the description is corrected.

USAGE:
  Connect between your existing ref controller and KSampler.
  ref_controller → ColorAnchor → KSampler
  No conditioning input needed — it reads the reference from conditioning
  the same way the other nodes do.
"""
